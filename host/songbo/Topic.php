...
class Topic extends YqBase {
	...
	// 按照标题关键字查询
	function queryTopicByKeyword($topic_keyword, $topic_time) {
		if ($this->yiquan_version == 0) {
			return - 2;
		}
	...
	// 按照label查询
	function queryTopicByLabel($topic_label, $topic_time) {
		if ($this->yiquan_version == 0) {
			return - 2;
		}
	...
	// 超级接口
	// getuserbyname,countmytopic,countmyrepliedtopic,countfirstfriend,countsecondfriend
	// function getProfileStats( $user_name ) {
	// $u = new User();
	// $result = json_decode($u->getUserByName( $user_name ));
	
	// $result['countTopic'] = countTopicByName( $user_name );
	
	// $r = new Reply();
	// $result['countMyReply'] = $r->countMyReplyByName( $user_name );
	
	// $result['countFirstFriends'] = $u->countFirstFriendsByName( $user_name );
	
	// $result['countSecondFriends'] = $u->countSecondFriendsByName( $user_name );
