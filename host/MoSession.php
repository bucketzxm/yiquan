...
class MoSession extends YqBase {
	...
	function classesBySessionID ($session_user_id){
		...
		$classes = $this->db->MoClass->find(array('session_id' => $session_id))->sort(array('class_num' => 1));
		$results = array();
	...
	function pinnedCardsByClassID($user_class_id){
		...
		$results = array();
		$resultsSection = array();
		$resultCards = array();
		foreach ($theCards as $keycard => $card) {
			array_push($resultCards, $card);
		}

		array_push($resultsSection,$resultCards);
		$results['cards'] = $resultsSection;
		$results['cursor'] = 0;
	...
	function addClassToPlan ($user_class_id){
		...
		$record = $this->db->MoStudy->findOne(array('student_id' => $user_id,'class_id' => $class_id,'study_type' =>'cursor'));
		if ($record == null) {
			$newRecord = array(
					'study_type' => 'cursor',
					'student_id' => $user_id,
