...
<div class="container">
	<form method="post"
		...
		<table>
			<tr>
				<td><h2 class="form-signin-heading">请填写信息</h2></td>
			</tr>
			<tr>
...
if (isset ( $_POST ['username'] ))
			...
			<tr>
				...
				<td><input class="form-control" type="password" name="password1"
					id="password1" value="" /></td>
			</tr>
			<tr>
				<td><label for="password2">Password Again</label></td>
				<td><input class="form-control" type="password" name="password2"
...
if (isset ( $_POST ['email'] ))
			...
			<tr>
				...
				<td>Enter text seen in this image<br/ > <img
					src="img/captcha.php?nocache=<?php echo time(); ?>" alt="" /><br />
					<input class="form-control" type="text" name="captcha" id="captcha" /></td>
			</tr>
			<tr>
				<td></td>
...
// otherwise process incoming data
	...
	if ($password && $captcha && User::validateUsername ( $_POST ['username'] ) && User::validateEmailAddr ( $_POST ['email'] )) {
		...
		$GLOBALS ['TEMPLATE'] ['content'] .= '<p><strong>You provided some ' . 'invalid data.</strong></p> <p>Please fill in all fields ' . 'correctly so we can register your user account.</p>';
		$GLOBALS ['TEMPLATE'] ['content'] .= $form;
	}
}
