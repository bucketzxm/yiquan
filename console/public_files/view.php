...
if ($forum_id)
{
    ...
    {    
        ...
        echo '<p><a href="view.php?fid=' . $forum_id . '">回到论坛帖子 ' . 
            '</a></p>';
    }
    else
    {    
        // link back to forum list
        echo '<p><a href="main.php">回到班级列表</a></p>';

        // display option to add new post if user is logged in
        if (isset($_SESSION['access']))
        {
            echo '<p><a href="add_post.php?fid=' . $forum_id . '">发新帖子</a></p>';
        }
    }
...
ENDSQL;
    ...
    while ($row = mysql_fetch_array($result))
    {
        ...
        if ($s2->fileExists('smallform','avatars/' . $row['USERNAME'] . '.jpg'))
        {
            echo '<img src="'.$s2->getUrl('smallform','avatars/' . $row['USERNAME'] . '.jpg').'" />';
        }
        else
        {
...
else if ($forum_id)
{
    ...
    if ($total = mysql_num_rows($result))
    {
        ...
        {
            echo '<div class="bs-example"><a href="view.php?fid=' . $forum_id . '&mid=' . 
                $row['MESSAGE_ID'] . '">';
            echo date('m/d/Y', $row['MESSAGE_DATE']) . ': ';
            echo htmlspecialchars($row['SUBJECT']) . '</a></div>';
        }
        //echo '</ul>';
