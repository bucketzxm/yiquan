...
class Seed extends YqBase{
	...
	function queryBizSeedPassed($configs = []) {
		$ans = [ ];
		if (empty ( $configs )) {
			$cus = $this->db->Proseed->find (array('seed_domain' => 'business','seed_editorRating' => array('$gt' => 0),'seed_dbWriteTime'=>array('$gt'=>(time()-86400*3))))->sort(array('seed_dbWriteTime' => -1));
			while ( $cus->hasNext () ) {
				$doc = $cus->getNext ();
				$ans [] = $doc;
			}
...
#Part for ProSingleSource
	...
	function loadSingleSource($sourceID){
		...
		//执行单个Source的Prohot
		$seeds = $this->db->Proseed->find(array('seed_hotness' => array('$gt' => 1),'seed_sourceID' =>$sourceID));
		                        ...
		                        if (count($keywords) > 6){
		                        	foreach ($titles as $key3 => $title_name) {
		    	                        ...
		    	                        if ( $this->find_same2($keywords, $title_name['seed_keywordDict'])==1) {//$title_name['seed_industry'] == $industry &&
		    	                            echo '<p>' .'same2' . $seed['title'] . '</p>';
		    	                            echo '<p>' .'same2' . $title_name['seed_title'] . '</p>';
		    	                            $same = true;
		    	                            break;
		    	                        }

		    	                        if ($this->find_same2($keywords, $title_name['seed_keywordDict'])==2) {//$title_name['seed_industry'] == $industry && 
		                            ...
		                            foreach ($titles as $key3 => $title_name) {
		                            	if ($this->find_same($title, $title_name['seed_title'])==true) {//$title_name['seed_industry'] == $industry && (
		        	                            echo '<p>' .'same1'. $seed['title'] . '</p>';
		        	                            echo '<p>' .'same1'. $title_name['seed_title'] . '</p>';
		        	                            $same = true;
		        	                            break;
		        	                        }
		                            ...
		                            foreach ($sourceTitles as $key4 => $sourceTitle_name) {
		                                if ( $this->find_same($title, $sourceTitle_name['seed_title'])==true) {//$title_name['seed_industry'] == $industry &&
		                                    echo '<p>' .'same1'. $seed['title'] . '</p>';
		                                    echo '<p>' .'same1'. $sourceTitle_name['seed_title'] . '</p>';
		                                    $same = true;
		                                    break;
		                                }
		                            }
