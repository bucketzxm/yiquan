...
class MyPlatform extends YqBase {
	protected $bcs_host = 'bcs.duapp.com';
	function getMyReport($configs = []) {
		$st = time ();
		$ed = time ();
		...
		while ( $sst <= $ed ) {
			...
			while ( $cus->hasNext () ) {
				$doc = $cus->getNext ();
				if ($doc ['quote_ownerID'] != 'system') {
					$activeuser [$doc ['quote_ownerID']] = 1;
				}
			}
			
			$ans ["$sst"] ['active'] ['activeusercount'] = count ( $activeuser );
			
			$ans ["$sst"] ['behaviour'] ['Quoteaddcount'] = $this->db->Quote->count ( array (
					'quote_time' => array (
							'$gte' => $sst,
							'$lt' => strtotime ( '+1 day', $sst ) 
					) 
			...
			$ans ["$sst"] ['behaviour'] ['Quotecount'] = $this->db->Quote->count ( array (
					'quote_time' => array (
							'$lt' => strtotime ( '+1 day', $sst ) 
					) 
			) );
			
			$ans ["$sst"] ['user'] ['regcount'] = $this->db->Quoteuser->count ( array (
					'user_regdate' => array (
							'$lt' => new MongoDate ( strtotime ( '+1 day', $sst ) ) 
					) 
			) );
			$ans ["$sst"] ['user'] ['regcounttoday'] = $this->db->Quoteuser->count ( array (
					'user_regdate' => array (
			...
			$ans ["$sst"] ['user'] ['contactscount'] = count ( $contacts );
			if ($ans ["$sst"] ['user'] ['regcount'] > 0) {
				$ans ["$sst"] ['active'] ['activeandregratio'] = floor ( $ans ["$sst"] ['active'] ['activeusercount'] / $ans ["$sst"] ['user'] ['regcount'] * 100 ) . '%';
			} else {
