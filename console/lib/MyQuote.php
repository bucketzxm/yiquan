...
class Quote extends YqBase {
	...
	function queryLikeNames($quote_id, $user_id) {
		...
		try {
			$quote = $this->db->Quote->findOne ( array (
					'_id' => new MongoId ( $quote_id ) 
			) );
	...
	function queryMyQuotes($user_id, $time) {
		...
		try {
			...
			foreach ( $res as $key => $value ) {
				$user_info = $this->db->Quoteuser->findOne ( array (
	...
	function querySquareQuotes($user_id, $time) {
		...
		foreach ( $myMobiles as $key => $value ) {
			$contact = $this->db->Quoteuser->findOne ( array (
					'user_mobile' => $value 
			) );
			if ($contact != null) {
	...
	// ================================================================================================================
	// ================================================================================================================
	function queryQuotes($configs = []) {
		$ans = [ ];
		if (empty ( $configs )) {
			$cus = $this->db->Quote->find ()->sort ( array (
					'quote_time' => - 1 
			) );
			while ( $cus->hasNext () ) {
				$doc = $cus->getNext ();
	...
	function deleteQuotes($Quoteid) {
		try {
			$this->db->Quote->remove ( array (
					'_id' => new MongoID ( $Quoteid ) 
			) );
			return 1;
		} catch ( Exception $e ) {
			return - 1;
