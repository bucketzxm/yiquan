	...
	* */
	...
	function htDeleteFriendById($user_idA, $user_idB) {
		try {
			...
			if (isset ( $rowb ['user_relationships'] [$user_idA->{'$id'}] )) {
				unset ( $rowb ['user_relationships'] [$user_idA->{'$id'}] );
			}
	...
	function htlistallUserPages($limit) {
		$cus = $this->db->user->find ()->sort ( array (
				'_id' => 1 
		) );
		$pages = [ ];
		$c = - 1;
	...
	function htListAllUsers($headid, $limit = 20) {
		$cus = $this->db->user->find ( array (
				'_id' => array (
						'$gte' => new MongoId ( $headid ) 
				) 
		) )->sort ( array (
				'_id' => 1 
		) )->limit ( $limit );
	...
	function htDeleteUserById($user_id) {
		try {
			...
			if (is_null ( $row ))
				return 0;
			if (isset ( $row ['user_relationships'] )) {
				foreach ( $row ['user_relationships'] as $k => $v ) {
					
					$this->htDeleteFriendById ( $row ['_id'], $v ['userb_id'] );
				}
			}
			$this->db->user->remove ( $row, array (
					"justOne" => true 
			) );
			return 1;
		} catch ( Exception $e ) {
	...
	function hteditUserBasicInfo($arr) {
		$row = $this->db->user->findOne ( array (
	...
	function hteditUserProfileInfo($arr) {
		...
		foreach ( $arr as $k => $v ) {
			if (startWith ( $k, 'userProfile' )) {
				$tp = explode ( "-", $k );
				$zhengli [$tp [1]] = $v;
			}
		}
		
		$this->htaddProfileById ( $arr ['_id'], json_encode ( $zhengli ) );
	}
	...
	function htchangePasswordByID($user_id, $user_newpwd) {
		try {
			$row = $this->db->user->findOne ( array (
					'_id' => new MongoId ( $user_id ) 
			) );
	...
	function htupdateUserpicByUserId($data, $user_id) {
		// var_dump ( $data );
		$row = $this->db->user->findOne ( array (
				'_id' => new MongoId ( $user_id ) 
		) );
	...
	function htgetBehaviorsByid($id, $configs = array('type'=>'days','value'=>7)) {
		$st = time ();
		$ed = time ();
		...
		while ( $cus->hasNext () ) {
			$doc = $cus->getNext ();
			if (! isset ( $res [date ( 'Y-M-d', $doc ['date']->sec )] )) {
				$res [date ( 'Y-M-d', $doc ['date']->sec )] = [ ];
			}
			if (! isset ( $res [date ( 'Y-M-d', $doc ['date']->sec )] [$doc ['class']] )) {
				$res [date ( 'Y-M-d', $doc ['date']->sec )] [$doc ['class']] = [ ];
			}
			
			if (! isset ( $res [date ( 'Y-M-d', $doc ['date']->sec )] ['statistics'] )) {
				$res [date ( 'Y-M-d', $doc ['date']->sec )] ['statistics'] = [ ];
				$res [date ( 'Y-M-d', $doc ['date']->sec )] ['statistics'] ['count'] = 0;
			}
			foreach ( $doc ['methods'] as $key => $v ) {
				$res [date ( 'Y-M-d', $doc ['date']->sec )] ['statistics'] ['count'] += $v;
				if (! isset ( $res [date ( 'Y-M-d', $doc ['date']->sec )] [$doc ['class']] [$key] )) {
					$res [date ( 'Y-M-d', $doc ['date']->sec )] [$doc ['class']] [$key] = $v;
				} else {
					$res [date ( 'Y-M-d', $doc ['date']->sec )] [$doc ['class']] [$key] += $v;
				}
			}
